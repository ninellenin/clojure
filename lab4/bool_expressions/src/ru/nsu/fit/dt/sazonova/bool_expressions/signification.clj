...
(defn get-all-variables
    ...
    [expression]
    (println "ge-all-variables" expression)
    (if (constant? expression)
        '()
        (if (variable? expression)
            (list (variable-name expression))
            (distinct (flatten (merge 
                (map (fn [expr] (get-all-variables expr)) (expression-argument expression))))))))
    ...
    (list
        ...
        [(fn [expression]
            (logic-not? expression))
                (fn [expression values]
                    (logic-not (significate (expression-argument expression) values)))]
        [(fn [expression]
            (logic-and? expression))
                (fn [expression values]
                    (apply logic-and (map (fn [expr] (significate expr values)) (expression-argument expression))))]
        [(fn [expression]
            (logic-or? expression))
                (fn [expression values]
                    (apply logic-or (map (fn [expr] (significate expr values)) (expression-argument expression))))]))
